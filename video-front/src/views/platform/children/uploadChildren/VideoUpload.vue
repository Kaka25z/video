<template>
    <div class="videoUpload">
        ...
        <div class="upload-body-content" :style="selectedVideo ? 'display: none;' : ''">
            ...
            <div class="footer-item">
                <span>上传视频，即表示您已同意 <a target="_blank">用户使用协议</a> 与 <a target="_blank">视频社区公约</a> ，请勿上传色情，反动等违法视频。</span>
            </div>
            <div class="footer-item">
...
export default {
    ...
    methods: {
        ...
        async handleVideoChange(event) {
            ...
            const maxSizeInBytes = 3000 * 1024 * 1024;
            if (!file) {
                return;
            }
            if (file.size <= maxSizeInBytes) {
                ...
                // 文件大小超出限制
                ElMessage.error("视频太大了，请缩小体积后再次尝试")
            }
        },
        ...
        async upload() {
            ...
            for (this.current; this.current < chunks.length; this.current++) {
                ...
                formData.append('index', this.current);
                ...
                this.progress = Math.round(((this.current + 1) / chunks.length) * 100);
            }
            this.progress = 100;
        },
