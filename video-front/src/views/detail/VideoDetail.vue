        ...
        <div class="video-container">
            <div class="left-container" :style="`width: ${playerSize.width}px;`">
                ...
                <PlayerWrap :videoUrl="video.videoUrl" :title="video.title" :duration="video.duration" :user="user"
                    ...
                    @resize="updatePlayerSize" @sendDm="sendDanmu" @next="next" :isFollowing="this.isFollowing"></PlayerWrap>
                <!-- 三连转发 -->
                <div class="video-toolbar-container">
                    <div class="video-toolbar-left">
                        ...
                        <div class="toolbar-left-item-wrap">
                            <div class="video-toolbar-left-item"
                                :class="{ 'on': this.$store.state.attitudeToVideo.coin > 0 }" @click="coinToVideo()">
                            ...
                            <div class="up-info--right">
                                ...
                                <div class="up-info__btn-panel">
                                    ...
                                    <VPopover popStyle="padding-top: 10px;">
                                        ...
                                        <template #reference>
                                            <div class="default-btn follow-btn following" v-if="isFollowing">
...
export default {
    ...
    methods: {
        ...
        async coinToVideo() {
            ...
            const originalCoin = this.$store.state.attitudeToVideo.coin;
            const formData = new FormData();
            formData.append("vid", Number(this.video.vid));
            const res = await this.$post("/video/coin", formData, {
                headers: { Authorization: "Bearer " + localStorage.getItem("token") }
            });
            if (!res.data.data) {
                return;
    ...
    watch: {
        ...
        "user.uid" : {
            handler(newVal) {
                if (newVal) {
                    this.checkFollow(this.user.uid);
                }
            },
        }
