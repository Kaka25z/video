    ...
    <div class="s-space">
        <div class="col-full">
            <div class="main-content">
                <div class="header">
                    <div class="text">
                        <span>{{ this.spaceId == this.$store.state.user.uid ? "我" : "TA" }}的粉丝</span>
                    </div>
                </div>
                <div class="content">
...
export default {
    ...
    methods: {
        async getFanList(uid) {
            ...
            this.fansList = res.data.data.fansUids;
        },
    ...
    async mounted() {
        ...
        if (this.spaceId == this.$store.state.user.uid) {
            await this.getFanList(this.$store.state.user.uid);
        } else {
            await this.getFanList(this.spaceId);
        }
    },
    watch: {
        async fansList(newList) {
            if (newList.length > 0) {
