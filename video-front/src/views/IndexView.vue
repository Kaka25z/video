<template>
    <div class="index">
        ...
        <div class="main__layout">
            <ChannelIndex v-if="showChannelIndex" />
            <div class="recommended-container" v-else>
                ...
                <div class="feed-roll-btn">
                    <div class="roll-btn" @click="RefreshVideo(); refreshTime++;">
...
export default {
    ...
    methods: {
        ...
        async getRecommendedVideos() {
            this.loadingRandom = true;
            const res = await this.$get("/video/recommend", {
        ...
        async getCumulativeRecommendedVideos() {
            this.loadingMore = true;
            let ids = this.vids.join(",");
            const res = await this.$get("/video/cumulative/recommend", {
                params: { vids: ids },
                headers: { Authorization: "Bearer " + localStorage.getItem("token") }
            });
        ...
        async handleScroll() {
            ...
            if (bottomDistance < 800 && this.hasMore && !this.loadingMore) {
                if (localStorage.getItem("token")) {
                    await this.getCumulativeRecommendedVideos();
                } else {
