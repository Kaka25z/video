        ...
        <div class="right-entry">
            ...
            <div class="v-popover-wrap">
                <VPopover :pop-style="this.$store.state.isLogin ? 'padding-top: 17px; margin-left: -100px;' : 'padding-top: 17px;'">
                    ...
                    <template #content>
                        <div style="height: 556.6px; width: 521.6px; overflow-y: auto;" v-if="this.$store.state.isLogin">
                            <div class="fav-list">
                                <div class="fav-list-names">
                                    <ul>
                                        <li class="fav-title" v-for="fav in this.favList" :key="fav.fid" @click="selectFav(fav)" :class="{ active: fav === selectedFav }">
                                            {{ fav.title }}
                                        </li>
                                    </ul>
                                </div>
                                <div class="fav-list-content">
                                    <div v-for="(item, index) in this.favVideos" :key="index">
                                        <el-card shadow="never" style="border: 0" class="card" @click="navigateTo(item.video.vid)" >
                                            <el-row class="fav-video">
                                                <el-col :span=6>
                                                    <div class="fav-video__cover">
                                                        <img :src="item.video.coverUrl" alt="">
                                                        <div class="video-duration">{{ formatTime(item.video.duration) }}</div>
                                                    </div>
                                                </el-col>
                                                <el-col :span=14>
                                                    <div class="fav-video__info">
                                                        <div class="fav-video__title" style="position: absolute; top: 0">{{ item.video.title }}</div>
                                                        <div class="video-card__info--owner" style="position: absolute; bottom: 0">
                                                            <i class="iconfont icon-uper" :style="''"></i>
                                                            <span class="fvideo-card__info--author">{{ item.user.nickname }}</span>
                                                        </div>
                                                    </div>
                                                </el-col>
                                            </el-row>
            ...
            <div class="v-popover-wrap">
                <VPopover :pop-style="this.$store.state.isLogin ? 'padding-top: 17px; margin-left: -50px;' : 'padding-top: 17px;'">
                    ...
                    <template #content>
                        <div style="height: 556.6px; width: 371.6px; overflow-y: auto;" v-if="this.$store.state.isLogin">
                            <div style="padding-top: 12px; padding-bottom: 12px">
                                <div v-for="(item, index) in this.userPlayHistory" :key="index" class="scrollbar">
                                    <el-card shadow="never" style="border: 0" class="card" @click="navigateTo(item.video.vid)" >
                                        <el-row class="fav-video">
                                            <el-col :span=6>
                                                <div class="fav-video__cover">
                                            ...
                                            <el-col :span=14>
                                                <div class="fav-video__info">
                                                    ...
                                                    <div class="video-card__info--owner" style="position: absolute; bottom: 0">
                                                        <i class="iconfont icon-uper" :style="''"></i>
                                                        <span class="fvideo-card__info--author">{{ item.user.nickname }}</span>
                                                        <span class="fvideo-card__info--time">{{ formatPlayTime(item.user_video.playTime) }}</span>
                                                    </div>
                                                </div>
                                            </el-col>
                                        </el-row>
...
<script>
    ...
    import { handleNum, handleLevel, highlightKeyword, handleTime } from '@/utils/utils.js';
    ...
    export default {
        ...
        methods: {
            ...
            async getFavList() {
                ...
                this.favList = list;
                this.selectedFav = this.favList[0];
                console.log("收藏夹列表: ", this.selectedFav);
            ...
            async getFavVideos() {
                ...
                if (!res.data) return;
                this.favVideos = res.data.data.map(item => ({
                    user: item.user,
                    video: item.video,
                }));
                console.log("收藏夹视频列表: ", this.favVideos);
            ...
            async getUserPlay() {
                ...
                console.log("用户播放历史: ", this.userPlayHistory);

            },

            navigateTo(vid) {
                this.openNewPage(`/video/${vid}`);
            },
            ...
            formatPlayTime(playTime) {
                ...
                const timeString = `${hours}:${minutes}`; // 获取小时和分钟部分
            ...
            selectFav(fav) {
                ...
                this.getFavVideos();
                console.log("Selected favorite: ", fav);
            },
...
.fvideo-card__info--author {
    flex-shrink: 0;
}
  
.fvideo-card__info--time {
    flex-shrink: 0;
}
