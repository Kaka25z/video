...
import com.video.core.entity.MsgUnread;
import com.video.core.entity.User;
import com.video.core.entity.dto.UserDTO;
import com.video.core.im.IMServer;
import com.video.core.mapper.FavoriteMapper;
import com.video.core.mapper.MsgUnreadMapper;
...
public class UserAccountServiceImpl implements UserAccountService {
    ...
    public CustomResponse register(String username, String password, String confirmedPassword) throws IOException {
        ...
        customResponse.setMessage("注册成功！欢迎加入我们的网站");
        return customResponse;
    }
    ...
    public CustomResponse login(String username, String password) {
        ...
        userDTO.setFollowsCount(followService.getFollowCount(user.getUid()));
    ...
    public CustomResponse adminLogin(String username, String password) {
        ...
        final_map.put("token", token);
        final_map.put("user", userDTO);
        customResponse.setMessage("欢迎回来");
        customResponse.setData(final_map);
        return customResponse;
    }
    ...
    @Override
    public CustomResponse adminUpdatePassword(Integer uid, String npw) {
        CustomResponse customResponse = new CustomResponse();
        if (npw == null || npw.length() == 0) {
            customResponse.setCode(500);
            customResponse.setMessage("密码不能为空");
        ...
        if (user == null) {
            customResponse.setCode(404);
            customResponse.setMessage("用户不存在");
            return customResponse;
        }
        ...
        UpdateWrapper<User> updateWrapper = new UpdateWrapper<>();
        updateWrapper.eq("uid", uid).set("password", encodedPassword);
        userMapper.update(null, updateWrapper);
