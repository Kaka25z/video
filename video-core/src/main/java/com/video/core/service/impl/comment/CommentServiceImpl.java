package com.video.core.service.impl.comment;

import com.baomidou.mybatisplus.core.conditions.query.QueryWrapper;
import com.baomidou.mybatisplus.core.conditions.update.UpdateWrapper;
import com.baomidou.mybatisplus.extension.plugins.pagination.Page;
import com.video.core.entity.*;
import com.video.core.im.IMServer;
import com.video.core.mapper.CommentMapper;
...
//        System.out.println(commentTreeList);
    ...
    @Override
    public List<Map<String, Object>> getAllComments(Integer page, Integer quantity) {

        String cacheKey = "comments:" + page +":" + "quantity:" + quantity;
        List<Map<String, Object>> commentInfoList = (List<Map<String, Object>>) redisUtil.getObject(cacheKey, List.class);

        if (commentInfoList == null) {
            Page<Comment> commentPage = new Page<>(page, quantity);
            QueryWrapper<Comment> wrapper = new QueryWrapper<>();
            commentPage = commentMapper.selectPage(commentPage, wrapper);
            List<Comment> comments = commentPage.getRecords();
        
            final List<Map<String, Object>> finalCommentInfoList = new ArrayList<>();
        
            comments.forEach(comment -> {
                Map<String, Object> commentInfo = new HashMap<>();
                String user = userService.getUserById(comment.getUid()).getNickname();
                String userAvatarUrl = userService.getUserById(comment.getUid()).getAvatar_url();
                String videoTitle = videoMapper.selectById(comment.getVid()).getTitle();
        
                commentInfo.put("comment", comment);
                commentInfo.put("user", user);
                commentInfo.put("userAvatarUrl", userAvatarUrl);
                commentInfo.put("videoTitle", videoTitle);
        
                finalCommentInfoList.add(commentInfo);
            });
        
            CompletableFuture.runAsync(() -> {
                redisUtil.setExObjectValue(cacheKey, finalCommentInfoList);
            }, taskExecutor);
    ...
    @Override
    public List<Map<String, Object>> getCommentsByUid(Integer uid, Integer page, Integer quantity) {
        List<Integer> userVids = videoService.getVideoIdsByUid(uid);

        Page<Comment> commentPage = new Page<>(page, quantity);
        QueryWrapper<Comment> wrapper = new QueryWrapper<>();
        wrapper.in("vid", userVids);
        ...
        comments.forEach(comment -> {
            ...
            String userAvatarUrl = userService.getUserById(comment.getUid()).getAvatar_url();
            String videoTitle = videoMapper.selectById(comment.getVid()).getTitle();
            String videoCoverUrl = videoMapper.selectById(comment.getVid()).getCoverUrl();
    
            commentInfo.put("comment", comment);
            commentInfo.put("user", user);
            commentInfo.put("userAvatarUrl", userAvatarUrl);
            commentInfo.put("videoTitle", videoTitle);
            commentInfo.put("videoCoverUrl", videoCoverUrl);
    
            finalCommentInfoList.add(commentInfo);
        });
    
        return finalCommentInfoList;
    }
}