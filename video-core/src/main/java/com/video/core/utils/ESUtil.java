...
public class ESUtil {
    ...
    public boolean deleteSearchWord(String keyword) {
        try {
            DeleteRequest request = new DeleteRequest.Builder().index("search_word").id(keyword).build();
            DeleteResponse deleteResponse = client.delete(request);
            return deleteResponse != null;
        } catch (IOException e) {
            log.error("删除ES搜索词文档时出错了：" + e);
            return false;
        }
    }
}
